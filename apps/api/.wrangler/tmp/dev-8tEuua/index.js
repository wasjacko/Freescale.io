...
// .wrangler/tmp/bundle-shoxMZ/strip-cf-connecting-ip-header.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260515.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260515.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
function createMobileAuthMiddleware() {
  return async (c, next) => {
...
__name(createMobileAuthMiddleware, "createMobileAuthMiddleware");

// src/mobile/supabase.ts
function createUserSupabaseClient(env2, accessToken) {
  return {
    request(path, init = {}) {
      const headers = {
      ...
      return fetch(new URL(path, env2.SUPABASE_URL).toString(), {
        ...init,
        headers
      });
    }
...
__name(createUserSupabaseClient, "createUserSupabaseClient");

// src/mobile/workspace.ts
var MobileRouteError = class extends Error {
...
__name(MobileRouteError, "MobileRouteError");
function memberRole(value) {
  return value === "owner" || value === "admin" ? value : "member";
}
__name(memberRole, "memberRole");
function embeddedWorkspace(value) {
...
__name(embeddedWorkspace, "embeddedWorkspace");
async function requireRows(response) {
  if (!response.ok) {
    throw new MobileRouteError("upstream_error", 502, "Impossible de charger les donn\xE9es.");
  }
  const payload = await response.json();
  return Array.isArray(payload) ? payload : [];
}
__name(requireRows, "requireRows");
...
async function resolveMobileWorkspaceContext(client, user) {
  ...
  const profileRows = await requireRows(profileResponse);
  const memberRows = await requireRows(memberResponse);
  const workspaces = memberRows.map(toWorkspace).filter((workspace) => workspace !== null);
  if (workspaces.length === 0) {
    throw new MobileRouteError("workspace_not_found", 404, "Aucun espace accessible.");
  }
  const profileRow = profileRows[0];
  const savedWorkspaceId = typeof profileRow?.active_workspace_id === "string" ? profileRow.active_workspace_id : null;
  const activeWorkspace = workspaces.find((workspace) => workspace.id === savedWorkspaceId) ?? workspaces[0];
  if (!activeWorkspace) {
    throw new MobileRouteError("workspace_not_found", 404, "Aucun espace accessible.");
  }
  return {
...
__name(resolveMobileWorkspaceContext, "resolveMobileWorkspaceContext");

// src/mobile/tasks.ts
var TASK_SELECT = "id,title,description,status,priority,due_at,completed_at,created_at,updated_at";
var PRIORITIES = /* @__PURE__ */ new Set(["low", "medium", "high", "urgent"]);
function taskStatus(value) {
  if (value === "todo" || value === "in_progress" || value === "awaiting_reply" || value === "done") {
    return value;
  }
  return "todo";
}
__name(taskStatus, "taskStatus");
function taskPriority(value) {
  return PRIORITIES.has(value) ? value : "medium";
}
__name(taskPriority, "taskPriority");
function toMobileTask(row) {
  const createdAt = typeof row.created_at === "string" ? row.created_at : (/* @__PURE__ */ new Date(0)).toISOString();
  return {
...
__name(toMobileTask, "toMobileTask");
async function requireTasks(response) {
  if (!response.ok) {
    throw new MobileRouteError("upstream_error", 502, "Impossible de charger les t\xE2ches.");
  }
  const rows = await response.json();
  return Array.isArray(rows) ? rows.map((row) => toMobileTask(row)) : [];
}
__name(requireTasks, "requireTasks");
async function listMobileTasks(client, workspaceId, onlyOpen = false) {
...
function todayDate(queryDate) {
  if (queryDate === void 0) {
    return { ok: true, value: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10) };
  }
...
function buildToday(tasks, date) {
  const open = tasks.filter((task) => task.status !== "done");
  const now = open.filter(
    (task) => task.priority === "urgent" || task.priority === "high" || task.dueAt !== null && task.dueAt.slice(0, 10) <= date
  );
...
function validateCreateTask(payload) {
  if (!payload || typeof payload !== "object") {
    return { ok: false, message: "Corps de requ\xEAte invalide." };
  }
  const input = payload;
  const title2 = typeof input.title === "string" ? input.title.trim() : "";
  if (!title2) {
    return { ok: false, message: "Le titre est requis." };
  }
  if (input.priority !== void 0 && !PRIORITIES.has(input.priority)) {
    return { ok: false, message: "Priorit\xE9 invalide." };
  }
  ...
  let dueAt = null;
  if (input.dueAt !== void 0 && input.dueAt !== null) {
    if (typeof input.dueAt !== "string") {
      return { ok: false, message: "\xC9ch\xE9ance invalide." };
    }
...
__name(validateCreateTask, "validateCreateTask");
async function createMobileTask(client, workspaceId, input) {
  ...
  const task = tasks[0];
  if (!task) {
    throw new MobileRouteError("upstream_error", 502, "Impossible de cr\xE9er la t\xE2che.");
  }
  return task;
}
__name(createMobileTask, "createMobileTask");
async function completeMobileTask(client, workspaceId, taskId) {
  const response = await client.request(
  ...
  const task = tasks[0];
  if (!task) {
    throw new MobileRouteError("task_not_found", 404, "T\xE2che introuvable.");
  }
  return task;
}
__name(completeMobileTask, "completeMobileTask");

// src/mobile/routes.ts
function createMobileRoutes() {
  const routes = new Hono2();
  routes.use("*", createMobileAuthMiddleware());
  const handleError = /* @__PURE__ */ __name((error3) => {
    if (error3 instanceof MobileRouteError) {
      return {
        status: error3.status,
        payload: { error: { code: error3.code, message: error3.message } }
      };
    }
    return {
      status: 502,
      payload: { error: { code: "upstream_error", message: "Impossible de charger les donn\xE9es." } }
    };
  }, "handleError");
  routes.get("/me", async (c) => {
    try {
      ...
      const failure = handleError(error3);
  ...
  routes.get("/tasks", async (c) => {
    try {
      const client = createUserSupabaseClient(c.env, c.get("mobileAccessToken"));
      const context2 = await resolveMobileWorkspaceContext(client, c.get("mobileUser"));
      const tasks = await listMobileTasks(client, context2.activeWorkspace.id);
      return c.json({ tasks });
    } catch (error3) {
      const failure = handleError(error3);
      return c.json(failure.payload, failure.status);
  ...
  routes.get("/today", async (c) => {
    ...
    try {
      const client = createUserSupabaseClient(c.env, c.get("mobileAccessToken"));
      const context2 = await resolveMobileWorkspaceContext(client, c.get("mobileUser"));
      const tasks = await listMobileTasks(client, context2.activeWorkspace.id, true);
  ...
  routes.post("/tasks", async (c) => {
    ...
    try {
      const client = createUserSupabaseClient(c.env, c.get("mobileAccessToken"));
      const context2 = await resolveMobileWorkspaceContext(client, c.get("mobileUser"));
      const task = await createMobileTask(client, context2.activeWorkspace.id, input.value);
  ...
  routes.patch("/tasks/:id/complete", async (c) => {
    try {
      const client = createUserSupabaseClient(c.env, c.get("mobileAccessToken"));
      const context2 = await resolveMobileWorkspaceContext(client, c.get("mobileUser"));
      const task = await completeMobileTask(client, context2.activeWorkspace.id, c.req.param("id"));
      return c.json({ task });
    } catch (error3) {
      const failure = handleError(error3);
      return c.json(failure.payload, failure.status);
...
__name(createMobileRoutes, "createMobileRoutes");
...
function parseStripeSignatureHeader(header) {
  ...
  const parts = header.split(",").reduce(
    ...
    { timestamp: Number.NaN, signatures: [] }
  );
  if (!Number.isFinite(parts.timestamp) || parts.signatures.length === 0)
    return null;
  return parts;
}
...
async function verifyStripeSignature(rawBody, signatureHeader, webhookSecret, toleranceSeconds = 300, nowSeconds = Math.floor(Date.now() / 1e3)) {
...
__name(periodEndToIso, "periodEndToIso");
function buildProfileUpdateForStripeEvent(event) {
...
__name(buildProfileUpdateForStripeEvent, "buildProfileUpdateForStripeEvent");
...
app.route("/v1", createMobileRoutes());
async function patchProfileFromWebhook(env2, profileUpdate) {
  if (!env2.SUPABASE_URL || !env2.SUPABASE_SERVICE_ROLE_KEY) {
    return { ok: false, error: "missing Supabase service env" };
  }
  const url = new URL("/rest/v1/profiles", env2.SUPABASE_URL);
...
__name(patchProfileFromWebhook, "patchProfileFromWebhook");
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260515.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260515.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-shoxMZ/middleware-insertion-facade.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260515.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-shoxMZ/middleware-loader.entry.ts
